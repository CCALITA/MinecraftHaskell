...
-- | Game UI mode
data GameMode = MainMenu | Playing | Paused | InventoryOpen | CraftingOpen | ChestOpen | FurnaceOpen | EnchantingOpen | DeathScreen
    ...
    GLFW.setMouseButtonCallback (whWindow wh) $ Just $ \_win button action _mods -> do
      ...
      case mode of
        ...
        EnchantingOpen -> when (action == GLFW.MouseButtonState'Pressed && button == GLFW.MouseButton'1) $ do
          ...
          let ndcX = realToFrac mx / fromIntegral winW * 2.0 - 1.0 :: Float
              ndcY = realToFrac my / fromIntegral winH * 2.0 - 1.0 :: Float
              -- Item slot: centered at (-0.2, -0.1), size 0.12
              ...
              -- Enchantment option buttons: 3 rows starting at (0.05, -0.25)
              optBtnX0 = 0.05 :: Float
              optBtnW  = 0.50 :: Float
              optBtnH  = 0.10 :: Float
                ...
                Just (ItemStack _item _cnt) -> do
                  ...
                  when (idx < length options) $ do
                    ...
                    when (xpLvl >= cost) $ do
                      ...
                      -- Store enchantment on the item's slot (use slot 0 for enchanting slot)
