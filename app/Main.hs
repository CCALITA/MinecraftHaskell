    ...
    let loop = do
          ...
          unless shouldClose $ do
            ...
            mChestInv <- case mode of
              ...
              _ -> pure Nothing
            let hudVerts = buildHudVertices inv miningProgress (plHealth player') (plHunger player') mode cursorItem craftGrid mChestInv debugInfo (fmap (\tb -> (tb, vp)) targetBlock)
...
buildHudVertices :: Inventory -> Float -> Int -> Int -> GameMode -> Maybe ItemStack -> CraftingGrid -> Maybe Inventory -> Maybe (V3 Float, Float, Float, Int, Int) -> Maybe (V3 Int, M44 Float) -> VS.Vector Float
buildHudVertices inv miningProgress health hunger mode cursorItem craftGrid mChestInv debugInfo targetInfo = VS.fromList $
  case mode of
    MainMenu -> menuVerts
    Paused   -> pauseVerts
