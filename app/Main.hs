  ...
  withWindow (ecWindowWidth config) (ecWindowHeight config) (ecWindowTitle config) $ \wh -> do
    ...
    let loop = do
          ...
          unless shouldClose $ do
            ...
            let hudVerts = buildHudVertices inv miningProgress (plHealth player') (plHunger player') (plAirSupply player') mode cursorItem craftGrid mChestInv mDispInv furnaceState debugInfo (fmap (\tb -> (tb, vp)) targetBlock) showSleepMsg damageFlash mouseNdcX mouseNdcY (plPos player') spawnPos dayNightVal playerXP
...
buildHudVertices :: Inventory -> Float -> Int -> Int -> Float -> GameMode -> Maybe ItemStack -> CraftingGrid -> Maybe Inventory -> Maybe Inventory -> FurnaceState -> Maybe DebugInfo -> Maybe (V3 Int, M44 Float) -> Bool -> Float -> Float -> Float -> V3 Float -> V3 Float -> DayNightCycle -> Int -> VS.Vector Float
buildHudVertices inv miningProgress health hunger airSupply mode cursorItem craftGrid mChestInv mDispInv furnaceState debugInfo targetInfo showSleepMsg damageFlash mouseX mouseY playerPos spawnPos dayNight playerXP = VS.fromList $
  case mode of
    MainMenu -> menuVerts
    Paused   -> pauseVerts
