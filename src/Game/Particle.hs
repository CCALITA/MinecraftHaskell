      ...
      in if cw > 0.01
         then let ndcX = cx / cw; ndcY = cy / cw
           in if abs ndcX < 1.5 && abs ndcY < 1.5
              then let V4 r g b _a = partColor p
                       lifeFrac = partLife p / partMaxLife p
                       alpha = lifeFrac * 0.9
                       sz = 0.008 * lifeFrac + 0.004
                       x0 = ndcX - sz; y0 = ndcY - sz; x1 = ndcX + sz; y1 = ndcY + sz
...
data WeatherParticleType = RainDrop | SnowFlake
  deriving stock (Show, Eq)

data WeatherParticle = WeatherParticle
  { wpPos  :: !(V3 Float)
  , wpType :: !WeatherParticleType
  } deriving stock (Show, Eq)

clampParticleXZ :: Float -> Float -> Float
clampParticleXZ radius v
  | v >  radius = v - 2 * radius
  | v < -radius = v + 2 * radius
  | otherwise    = v

clampParticleY :: Float -> Float -> Maybe Float
clampParticleY height dy
  | dy < 0     = Nothing
  | dy > height = Just (dy - height)
  | otherwise    = Just dy

spawnWeatherParticles :: V3 Float -> BiomeType -> Int -> IO [WeatherParticle]
spawnWeatherParticles (V3 px py pz) biome count =
  replicateM count $ do
    ang <- randomRIO (0, 2 * pi :: Float)
    rad <- randomRIO (0, weatherParticleRadius)
    let ox = rad * cos ang; oz = rad * sin ang
    oy <- randomRIO (0, weatherParticleHeight)
    let pType = if isSnowBiome biome then SnowFlake else RainDrop
    pure WeatherParticle { wpPos = V3 (px + ox) (py + oy) (pz + oz), wpType = pType }

tickWeatherParticles :: Float -> V3 Float -> BiomeType -> [WeatherParticle] -> IO [WeatherParticle]
tickWeatherParticles dt (V3 px py pz) biome particles =
  mapM (\wp -> do
    let speed = case wpType wp of { RainDrop -> rainFallSpeed; SnowFlake -> snowFallSpeed }
        V3 wx wy wz = wpPos wp
        wy' = wy - speed * dt
        dx  = clampParticleXZ weatherParticleRadius (wx - px)
        dz  = clampParticleXZ weatherParticleRadius (wz - pz)
        wp' = wp { wpPos = V3 (px + dx) wy' (pz + dz) }
        dy  = wy' - py
    case clampParticleY weatherParticleHeight dy of
      ...
      Nothing -> do
        ...
        rad <- randomRIO (0, weatherParticleRadius)
        let ox = rad * cos ang; oz = rad * sin ang
            pType = if isSnowBiome biome then SnowFlake else RainDrop
        pure WeatherParticle { wpPos = V3 (px + ox) (py + weatherParticleHeight) (pz + oz), wpType = pType }
    ) particles
         ...
         then let ndcX = cx / cw; ndcY = cy / cw
           in if abs ndcX < 1.5 && abs ndcY < 1.5
              then case wpType wp of
                RainDrop ->
                  let r = 0.5; g = 0.6; b = 1.0; a = 0.4; hw = 0.001; hh = 0.012
                      x0 = ndcX - hw; x1 = ndcX + hw; y0 = ndcY - hh; y1 = ndcY + hh
                  in [x0,y0,r,g,b,a, x1,y0,r,g,b,a, x1,y1,r,g,b,a, x0,y0,r,g,b,a, x1,y1,r,g,b,a, x0,y1,r,g,b,a]
                SnowFlake ->
                  let r = 1.0; g = 1.0; b = 1.0; a = 0.7; sz = 0.004
                      x0 = ndcX - sz; x1 = ndcX + sz; y0 = ndcY - sz; y1 = ndcY + sz
                  in [x0,y0,r,g,b,a, x1,y0,r,g,b,a, x1,y1,r,g,b,a, x0,y0,r,g,b,a, x1,y1,r,g,b,a, x0,y1,r,g,b,a]
              else []
         else []
