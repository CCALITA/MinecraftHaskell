...
slabBlockSpec = describe "Slab blocks (StoneSlab & OakSlab)" $ do
  ...
  -- Enum roundtrip
  it "slab block types have correct enum values" $ do
    fromEnum StoneSlab `shouldBe` 41
    fromEnum OakSlab `shouldBe` 42
    toEnum 41 `shouldBe` (StoneSlab :: BlockType)
    toEnum 42 `shouldBe` (OakSlab :: BlockType)
