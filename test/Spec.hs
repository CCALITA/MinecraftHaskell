...
weatherParticleSpec = describe "Game.Particle weather" $ do
  ...
  describe "renderWeatherParticles" $ do
    ...
    it "rain particle produces 36 floats" $ do
      let verts = renderWeatherParticles [WeatherParticle (V3 0 0 (-1)) RainDrop] identity
      length verts `shouldBe` 36
    it "snow particle produces 36 floats" $ do
      let verts = renderWeatherParticles [WeatherParticle (V3 0 0 (-1)) SnowFlake] identity
      length verts `shouldBe` 36
    it "rain has blue color" $ do
      let verts = renderWeatherParticles [WeatherParticle (V3 0 0 (-1)) RainDrop] identity
      case verts of { (_:_:r:g:b:_) -> do { r `shouldBe` 0.5; g `shouldBe` 0.6; b `shouldBe` 1.0 }; _ -> expectationFailure "no verts" }
    it "snow has white color" $ do
      let verts = renderWeatherParticles [WeatherParticle (V3 0 0 (-1)) SnowFlake] identity
      case verts of { (_:_:r:g:b:_) -> do { r `shouldBe` 1.0; g `shouldBe` 1.0; b `shouldBe` 1.0 }; _ -> expectationFailure "no verts" }
