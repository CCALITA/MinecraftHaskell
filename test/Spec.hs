...
inventorySpec = describe "Game.Inventory" $ do
  ...
  it "addItem places items in first empty slot" $ do
    let (inv, remaining) = addItem emptyInventory Stone 10
    remaining `shouldBe` 0
    getSlot inv 0 `shouldBe` Just (ItemStack Stone 10)

  it "addItem merges into existing stacks" $ do
    let (inv1, _) = addItem emptyInventory Stone 32
        (inv2, _) = addItem inv1 Stone 16
    getSlot inv2 0 `shouldBe` Just (ItemStack Stone 48)

  it "addItem respects stack limit of 64" $ do
    let (inv1, _) = addItem emptyInventory Stone 64
        (inv2, _) = addItem inv1 Stone 10
    getSlot inv2 0 `shouldBe` Just (ItemStack Stone 64)
    getSlot inv2 1 `shouldBe` Just (ItemStack Stone 10)

  it "addItem returns leftover when inventory is full" $ do
    let fillAll inv 0 = inv
        fillAll inv n = let (inv', _) = addItem inv Stone 64
                        in fillAll inv' (n - 1)
        full = fillAll emptyInventory inventorySlots
        (_, leftover) = addItem full Stone 10
    leftover `shouldBe` 10

  it "removeItem removes from end first" $ do
    let (inv1, _) = addItem emptyInventory Stone 32
        (inv2, _) = addItem inv1 Dirt 16
        (inv3, removed) = removeItem inv2 Stone 10
    removed `shouldBe` 10
    getSlot inv3 0 `shouldBe` Just (ItemStack Stone 22)

  it "removeItem returns actual count removed" $ do
    let (inv1, _) = addItem emptyInventory Stone 5
  ...
  it "selectedItem returns item in active hotbar slot" $ do
    ...
    selectedItem inv2 `shouldBe` Just (ItemStack Stone 10)
...
craftingSpec = describe "Game.Crafting" $ do
  ...
  it "single OakLog produces 4 OakPlanks" $ do
    let grid = setCraftingSlot (emptyCraftingGrid 3) 0 0 (Just OakLog)
    tryCraft grid `shouldBe` CraftSuccess OakPlanks 4
  ...
  it "2x2 planks produces CraftingTable" $ do
    let grid = setCraftingSlot
              ...
              (setCraftingSlot (emptyCraftingGrid 3)
                ...
                1 0 (Just OakPlanks))
                1 1 (Just OakPlanks)
    tryCraft grid `shouldBe` CraftSuccess CraftingTable 1

  it "recipe works regardless of position in grid" $ do
    -- OakLog in center should still produce planks
    let grid = setCraftingSlot (emptyCraftingGrid 3) 1 1 (Just OakLog)
    tryCraft grid `shouldBe` CraftSuccess OakPlanks 4
  ...
  it "2x2 grid can craft simple recipes" $ do
    let grid = setCraftingSlot (emptyCraftingGrid 2) 0 0 (Just OakLog)
    tryCraft grid `shouldBe` CraftSuccess OakPlanks 4
